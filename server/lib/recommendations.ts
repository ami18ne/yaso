...
export class RecommendationEngine {
  ...
  async getRecommendedPosts(userId: string, limit: number = 20): Promise<string[]> {
    try {
  ...
  async getRecommendedVideos(userId: string, limit: number = 20): Promise<string[]> {
    try {
  ...
  async getRecommendedUsers(userId: string, limit: number = 10): Promise<string[]> {
    try {
      const client = this.checkSupabase();
      ...
      const likedAuthors = (interactedUsers || [])
        .map((i: any) => i.posts?.user_id)
        .filter(Boolean);
