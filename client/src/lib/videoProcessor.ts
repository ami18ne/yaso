...
class VideoProcessor {
  ...
  async mergeAudioWithVideo(videoFile: File, audioUrl: string, audioVolume: number = 0.5): Promise<Blob> {
    if (!this.ffmpeg || !this.loaded) {
      await this.load()
    }
    ...
    await this.ffmpeg!.exec([
      '-i', inputVideoName,
      '-i', inputAudioName,
      '-filter_complex', `[0:a]volume=1[a0];[1:a]volume=${audioVolume}[a1];[a0][a1]amix=inputs=2:duration=first[a]`,
      '-map', '0:v',
      '-map', '[a]',
      '-c:v', 'copy',
      '-shortest',
  ...
  async processVideo(
    ...
    if (options.filterId && options.filterId !== 'none') {
      processedBlob = await this.applyFilter(
        processedBlob instanceof File ? processedBlob : new File([processedBlob], 'video.mp4', { type: 'video/mp4' }),
        options.filterId
      )
    }
    
    if (options.audioUrl) {
      const videoFileToProcess = processedBlob instanceof File 
        ? processedBlob 
  ...
  async extractThumbnail(videoFile: File, timeInSeconds: number = 1): Promise<Blob> {
    if (!this.ffmpeg || !this.loaded) {
      await this.load()
    }
