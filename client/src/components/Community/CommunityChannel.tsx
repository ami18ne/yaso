...
export default function CommunityChannel({ communityId, channelId, userId }: { communityId: string; channelId: string; userId?: string | null }) {
  ...
  usePusher(`community-${communityId}-channel-${channelId}`, handleNewMessage, { isConversation: false, eventName: 'new-channel-message' })
      ...
      <div className="flex-1 overflow-auto p-4">
        {localMessages.map(msg => (
          <div key={msg.id} className="mb-2">
            <MessageBubble id={msg.id} text={msg.content} isSent={msg.sender_id === userId} timestamp={msg.created_at} senderName={msg.sender?.username} senderAvatar={msg.sender?.avatar_url} />
          </div>
        ))}
      </div>
