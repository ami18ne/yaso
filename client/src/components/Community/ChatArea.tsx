...
const ChatArea = ({ channelId, channelName = 'channel' }: ChatAreaProps) => {
  ...
  if (error) {
    return <div className="flex-1 bg-[#313439] flex items-center justify-center text-red-400">Failed to load messages.</div>;
  }
    ...
    <div className="flex-1 bg-[#313439] flex flex-col h-screen">
      ...
      <div className="flex-shrink-0 px-4 pb-4">
        <div className="bg-[#3A3E45] rounded-lg flex items-center px-3">
          ...
          />
          <Button size="icon" variant="ghost" onClick={handleSendMessage} disabled={sendMessageMutation.isPending}>
...
const Message = ({ message }: { message: any }) => {
  ...
  return (
    <div className="flex items-start space-x-3">
        <img src={sender.avatar} alt={sender.name} className="w-10 h-10 rounded-full bg-neutral-700" />
        <div>
