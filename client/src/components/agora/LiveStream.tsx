...
import { Mic, MicOff, Video, VideoOff, Phone, ScreenShare, ScreenShareOff } from "lucide-react";
...
const HostView: React.FC<{ channelName: string; onEndStream: () => void; }> = ({ channelName, onEndStream }) => {
...
const AudienceView: React.FC<{ channelName: string; onLeave: () => void; }> = ({ channelName, onLeave }) => {
      ...
      <div className="flex-grow p-4 flex items-center justify-center">
        {host && host.hasVideo ? (
          <div className="relative w-full h-full">
            <RemoteUser user={host} playVideo={true} playAudio={true} className="w-full h-full object-contain" />
            <div className="absolute top-2 left-2 z-10 bg-black bg-opacity-50 px-2 py-1 rounded text-sm">
              Live Stream
            </div>
          </div>
          ...
          <div className="text-center">
            ...
            <p className="text-gray-400 mt-2">The live stream will appear here once the host begins broadcasting.</p>
          </div>
        )}
      </div>
