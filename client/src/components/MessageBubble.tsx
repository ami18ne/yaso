...
export default function MessageBubble({
  ...
  const handleToggleReaction = async (reaction = 'heart') => {
    try {
      ...
      ErrorLogger.log('Failed to toggle reaction', err)
    }
  }
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          'flex flex-col gap-1',
          (isSent && !isRTL) || (!isSent && isRTL) ? 'items-end' : 'items-start'
        )}
      >
        {!isSent && senderName && (
          <span
            className={`text-xs text-muted-foreground px-3 ${isRTL ? 'text-right' : 'text-left'}`}
          >
            {senderName}
          </span>
        )}
        ...
        <div
          className={`flex items-center gap-1.5 px-3 ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}
        >
          <span className="text-xs text-muted-foreground">{timestamp}</span>
          {isSent && <span className="text-xs text-primary">{isRead ? '✓✓' : '✓'}</span>}
          <button
